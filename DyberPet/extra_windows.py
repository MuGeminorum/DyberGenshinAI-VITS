...
class Tomato(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.button_close.setIcon(
            QIcon(os.path.join(basedir, "res/icons/close_icon.png"))
        )
        # self.button_close.setIconSize(QSize(int(20*size_factor), int(20*size_factor)))
        ...
        self.option_label.setFixedSize(
            int(20 * len(self.config["option_text"])), int(50)
        )
...
class Remindme(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        if os.path.isfile(os.path.join(configdir, "data/remindme.txt")):
            f = open(
                os.path.join(configdir, "data/remindme.txt"), "r", encoding="UTF-8"
            )
            texts = f.read()
            f.close()
            texts = texts.lstrip("\n")
            ...
            f = open(
                os.path.join(configdir, "data/remindme.txt"), "w", encoding="UTF-8"
            )
            f.write("")
            f.close()
        ...
        for line in texts:
            ...
            if line.startswith(self.tr("#重复")):
                ...
                if line[1] == self.tr("每到"):
                    ...
                    self.confirm_remind.emit(
                        "repeat_interval", 0, int(line[2]), line[-1]
                    )
        ...
        if self.checkA.isChecked():
            ...
            if self.check1.isChecked() and self.every_min.text() != "":
                current_text += (
                    self.tr("#重复")
                    + " "
                    + f"{self.tr('每到')} {int(self.every_min.text())} {self.tr('分时')} - {remind_text}\n"
                )
                self.confirm_remind.emit(
                    "repeat_point", 0, int(self.every_min.text()), remind_text
                )
                ...
                current_text += (
                    self.tr("#重复")
                    + " "
                    + f"{self.tr('每隔')} {int(self.interval_min.text())} {self.tr('分钟')} - {remind_text}\n"
                )
                self.confirm_remind.emit(
                    "repeat_interval", 0, int(self.interval_min.text()), remind_text
                )
    ...
    def __init__(self, cell_index, item_config=None, item_num=0):
        ...
        if item_config is not None:
            ...
            self.image = self.image.scaled(
                self.size_wh, self.size_wh, mode=Qt.SmoothTransformation
            )
            self.setPixmap(QPixmap.fromImage(self.image))
            self.installEventFilter(ToolTipFilter(self, showDelay=500))
            self.setToolTip(item_config["hint"])
            if self.item_config.get("item_type", "consumable") in [
                "collection",
                "dialogue",
            ]:
                ...
                self.setStyleSheet(
                    ItemStyle
                )  # "QLabel{border : 3px solid #4c9bf7; border-radius: 5px}")
        else:
            self.setStyleSheet(
                EmptyStyle
            )  # "QLabel{border : 3px solid #6d6f6d; border-radius: 5px}")
...
class Inventory(QWidget):
    ...
    def __init__(self, items_data, parent=None):
        ...
        self.tab_widget.addTab(
            self.FoodScrollArea, QIcon(os.path.join(basedir, "res/icons/tab_1.svg")), ""
        )
        self.tab_widget.addTab(
            self.ClctScrollArea, QIcon(os.path.join(basedir, "res/icons/tab_2.svg")), ""
        )
        self.tab_widget.setIconSize(QSize(int(30), int(20)))
        # self.tab_widget.setIconSize(QSize(int(30*size_factor), int(20*size_factor)))

        self.layer_dict = {
            "consumable": self.FoodGridLayout,
    ...
    def construct_item_tab(self, item_types, tab_index):
        ...
        keys = [
            i for i in keys if self.items_data.item_dict[i]["item_type"] in item_types
        ]
        if tab_index == 0:
        ...
        for item in keys:
            ...
            n_col = (index_item - (n_row - 1) * self.inven_shape[1]) % self.inven_shape[
                1
            ]

            self.items_numb[(tab_index, index_item)] = int(
                settings.pet_data.items[item]
            )
            self.cells_dict[(tab_index, index_item)] = Inventory_item(
                (tab_index, index_item),
                self.items_data.item_dict[item],
                self.items_numb[(tab_index, index_item)],
            )
            self.cells_dict[(tab_index, index_item)].Ii_selected.connect(
                self.change_selected
            )
            self.cells_dict[(tab_index, index_item)].Ii_removed.connect(
                self.item_removed
            )
        ...
        if index_item < self.inven_shape[0] * self.inven_shape[1]:
            ...
            for j in range(index_item, (self.inven_shape[0] * self.inven_shape[1])):
                ...
                self.cells_dict[(tab_index, j)].Ii_selected.connect(
                    self.change_selected
                )
    ...
    def add_item(self, item_name, n_items):
        ...
        if item_exist:
            ...
            n_col = (item_index - (n_row - 1) * self.inven_shape[1]) % self.inven_shape[
                1
            ]
            ...
            self.cells_dict[item_index] = Inventory_item(
                item_index, self.items_data.item_dict[item_name], n_items
            )
            ...
            self.layer_dict[item_type].addWidget(
                self.cells_dict[item_index], n_row, n_col
            )
...
class QToaster(QFrame):
    ...
    def checkClosed(self):
        ...
        if (
            self.opacityAni.direction() == QAbstractAnimation.Backward
        ):  # self.opacityAni.Backward:
            self._closeit("faded")
    ...
    def setupMessage(
        ...
        if corner == Qt.TopLeftCorner:
            geo.moveTopLeft(
                parentRect.topLeft()
                + QPoint(self.margin, self.margin + self.height_margin)
            )
...
class DPDialogue(QWidget):
    ...
    def OptionGenerator(self, text_key=None, prev_text=None, reverse=False):
        for item in [
            self.OptionLayout.itemAt(i) for i in range(self.OptionLayout.count())
        ]:
            # item.deleteLater()
            widget = item.widget()
            widget.deleteLater()
        ...
        if prev_text is not None and not reverse:
            if text_key is not None:
                ...
                if "option_prev_%s" % text_key not in self.message["relationship"].get(
                    text_key, []
                ):
                    self.message["option_prev_%s" % text_key] = self.tr("Back")
                    self.message["relationship"][text_key] = self.message[
                        "relationship"
                    ].get(text_key, []) + ["option_prev_%s" % text_key]
            else:
                ...
                self.opts_dict[option_index] = DialogueButtom(
                    self.tr("Back"), "option_prev_end"
                )  ##################
    ...
    def confirm(self):
        ...
        if new_key == []:
            ...
            self.OptionGenerator(
                prev_text=self.text_now, reverse=self.sender().msg == self.tr("Back")
            )
            self.text_now = ""
        else:
            new_key = new_key[0]
            ...
            self.OptionGenerator(
                new_key, self.text_now, reverse=self.sender().msg == self.tr("Back")
            )
