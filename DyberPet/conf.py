...
class PetConfig:
    ...
    def init_config(cls, pet_name: str, pic_dict: dict):
        ...
        with open(path, "r", encoding="UTF-8") as f:
            ...
            act_path = os.path.join(
                basedir, "res/role/{}/act_conf.json".format(pet_name)
            )
            act_conf = dict(json.load(open(act_path, "r", encoding="UTF-8")))
            act_dict = {}
            # with open(act_path, 'r', encoding='UTF-8') as f:
            act_dict = {
                k: Act.init_act(v, pic_dict, o.scale, pet_name)
            ...
            msg_file = os.path.join(
                basedir, "res/role/{}/msg_conf.json".format(pet_name)
            )
    ...
    def init_sys(cls, pic_dict: dict):
        ...
        with open(path, "r", encoding="UTF-8") as f:
            ...
            act_dict = {}
            act_dict = {
                k: Act.init_act(v, pic_dict, o.scale, "sys")
                for k, v in act_conf.items()
            }
    ...
    def init_subpet(cls, pet_name: str, pic_dict: dict):
        ...
        with open(path, "r", encoding="UTF-8") as f:
            ...
            # 初始化所有动作
            act_path = os.path.join(
                basedir, "res/pet/{}/act_conf.json".format(pet_name)
            )
            act_conf = dict(json.load(open(act_path, "r", encoding="UTF-8")))
            act_dict = {}
            act_dict = {
                k: Act.init_act(v, pic_dict, o.scale, pet_name, "pet")
                for k, v in act_conf.items()
            }
    ...
    for action, actDic in act_conf.items():
        if "images" not in actDic.keys():
            ...
            imgExist = [
                ...
                if not os.path.exists(f"{img_dir}_{i}.png")
            ]
            if imgExist == []:
                pass
            else:
...
class Act:
    ...
    def init_act(cls, conf_param, pic_dict, scale, pet_name, resFolder="role"):
        ...
        img_dir = os.path.join(
            basedir, "res/{}/{}/action/{}".format(resFolder, pet_name, images)
        )
        ...
        return Act(
            img, act_num, need_move, direction, frame_move, frame_refresh, anchor
        )
...
class PetData:
    ...
    def init_data(self):
        if os.path.isfile(self.file_path):
            ...
            if self.current_pet in allData_params.keys():
                ...
                now = datetime.now()
                allData_params[self.current_pet] = {
                    "HP": -1,
                    "HP_tier": 3,
                    "FV": 0,
                    "FV_lvl": 0,
    ...
    def check_save_integrity(self, save_allDict, petname):  # , days_info=False):
        ...
        if "HP" in save_allDict:
            ...
            try:
                check_key_value = all(
                    key in save_dict
                    and isinstance(save_dict[key], self.value_type[key])
                    for key in self.value_type.keys()
                )
            except:
                return 0
    ...
    def transfer_save_toPet(self, data_params, petname):
        ...
        if petname not in self.allData_params.keys():
            ...
            days, last_opened = (
                self.allData_params[petname]["days"],
                self.allData_params[petname]["last_opened"],
            )
            self.allData_params[petname] = data_params.copy()
        ...
        if self.taskData["history"]:
            ...
            if (now - last_opened).days == 0:
                ...
                if len(self.taskData["history"]) >= 2:
                    ...
                    last_2nd_opened = datetime(
                        year=int(last_2nd[0]),
                        month=int(last_2nd[1]),
                        day=int(last_2nd[2]),
                        hour=now.hour,
                        minute=now.minute,
                        second=now.second,
...
class ItemData:
    ...
    def init_data(self):
        ...
        for i, itemFolder in enumerate(sorted_itemMods):
            ...
            MOD_dict = {
                k: self.init_item(v, k, itemFolder, modName)
                for k, v in item_conf.items()
            }
    ...
    def init_item(self, conf_param, itemName, itemFolder, modName):
        ...
        description = text_wrap(
            conf_param.get("description", ""), 15
        )  # self.wrapper(conf_param.get('description', ''))
        item_type = conf_param.get("type", "consumable")

        buff = conf_param.get("buff", {})

        if effect_FV == 0 and effect_HP == 0:
            hint = "{} {}\n\n{}\n".format(name, " ".join(["⭐"] * fv_lock), description)
        else:
...
def load_ItemMod(configPath, HUNGERSTR="Satiety", FAVORSTR="Favorability"):
    ...
    return {
        k: init_item(v, k, itemFolder, modName, HUNGERSTR, FAVORSTR)
        for k, v in item_conf.items()
    }
...
def checkItemMOD(itemFolder):
    ...
    missingImage = [
        ...
        if not os.path.exists(os.path.join(itemFolder, v["image"]))
    ]
    if missingImage:
    ...
    typeMissmatch = [
        k for k, v in item_dict.items() if type(v.get("pet_limit", [])) is not list
    ]
    if typeMissmatch:
