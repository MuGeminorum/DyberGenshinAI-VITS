...
class DPNote(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        sys_note_conf = dict(
            json.load(
                open(
                    os.path.join(basedir, "res/icons/note_icon.json"),
                    "r",
                    encoding="UTF-8",
                )
            )
            ...
            pet_note_conf = dict(
                json.load(
                    open(
                        os.path.join(
                            basedir,
                            "res/role/{}/note/note.json".format(settings.petname),
                        ),
                        "r",
                        encoding="UTF-8",
                    )
        ...
        pet_cof = dict(
            json.load(
                open(
                    os.path.join(
                        basedir, "res/role/{}/pet_conf.json".format(settings.petname)
                    ),
                    "r",
                    encoding="UTF-8",
                )
            ...
            if k in pet_note_conf.keys():
                if "image" in pet_note_conf[k].keys():
                    img_file = os.path.join(
                        basedir,
                        "res/role/{}/note/{}".format(
                            settings.petname, pet_note_conf[k]["image"]
                        ),
                    )
                else:
                    img_file = os.path.join(
                        basedir,
                        "res/icons/{}".format(
                            sys_note_conf[k].get("image", "icon.png")
                        ),
                    )
                ...
                if "sound" in pet_note_conf[k].keys():
                    url = os.path.join(
                        basedir,
                        "res/role/{}/note/{}".format(
                            settings.petname, pet_note_conf[k]["sound"]
                        ),
                    )  # QUrl.fromLocalFile('res/role/{}/note/{}'.format(settings.pet_data.petname, pet_note_conf[k]['sound']))
                else:
                    url = os.path.join(
                        basedir,
                        "res/sounds/{}".format(
                            sys_note_conf[k].get("sound", "Notification.wav")
                        ),
                    )  # QUrl.fromLocalFile('res/sounds/{}'.format(sys_note_conf[k].get('sound', '13945.wav')))
                ...
                img_file = os.path.join(
                    basedir,
                    "res/icons/{}".format(sys_note_conf[k].get("image", "icon.png")),
                )
    ...
    def setup_notification(self, note_type, message=""):
        ...
        if note_type in self.icon_dict.keys():
            ...
            random_list = [
                i
                for i in self.icon_dict.keys()
                if i.startswith("random")
                and self.icon_dict[i]["fv_lock"] <= settings.pet_data.fv_lvl
            ]
            # print(random_list)
        ...
        if message != "":
            height_margin = sum(self.height_dict.values()) + 10 * (
                len(self.height_dict.keys())
            )
            self.note_dict[note_index] = DyberToaster(
                note_index,
                message=message,
                icon=icon,
    ...
    def fvchange_note(self, fv_lvl):
        ...
        if fv_lvl == -1:
            self.setup_notification(
                "status_fv",
                message=self.tr(
                    "Congrats! You have reached the max FV level! Thank you for your companionship all this time!"
                ),
            )
        else:
            self.setup_notification(
