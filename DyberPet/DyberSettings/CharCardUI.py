...
class CharInterface(ScrollArea):
    ...
    def __init__(self, sizeHintDyber, parent=None):
        ...
        self.CharListLink = HyperlinkButton(
            settings.CHARCOLLECT_LINK, self.tr("Collected Characters"), self, FIF.LINK
        )
        ...
        self.instructButton = TransparentPushButton(
            self.tr("Add Chars Manually"), self, FIF.QUESTION
        )
    ...
    def __initCardLayout(self):
        ...
        for i, character in enumerate(petlist):
            ...
            if not os.path.exists(infoFile):
                ...
                card = CharCard(
                    i, jsonPath=infoFile, petFolder=character
                )  # , parent=self.CharCardGroup)
    ...
    def _finishStateTooltip(self):
        if not self.launchTooltip:
            ...
            self.launchTooltip.setContent(self.tr("Launched!") + " 😆")
    ...
    def __onAddClicked(self):
        ...
        content = self.tr(
            "You are about to import a character from a local file. Please be aware that it is from third-party sources. We are not responsible for any potential harm or issues that may arise from using this character. Only proceed if you trust the source."
        )
        if not self.__showMessageBox(title, content):
            return
        ...
        folder = QFileDialog.getExistingDirectory(
            self,
            self.tr("Please select the character folder"),
    ...
    def __onAddClickedContinue(self):
        ...
        if not os.path.exists(infoFile):
            ...
            card = CharCard(
                iCard, jsonPath=infoFile, petFolder=petFolder
            )  # , parent=self.CharCardGroup)
            self.CharCardList.append(card)
    ...
    def _stopStateTooltip(self, success=True):
        if not self.stateTooltip:
            ...
            self.stateTooltip.setContent(self.tr("Copy complete!") + " 😆")
    ...
    def _send_CharImportResult(self, statCode, errorList):
        ...
        stat_notes = [
            ...
            self.tr(
                "The following actions called by pet_conf.json are missing from act_conf.json:"
            ),
        ]
