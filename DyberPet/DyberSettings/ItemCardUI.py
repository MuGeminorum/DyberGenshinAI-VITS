...
from .custom_utils import CharCardGroup, ItemLine, ItemCard
...
class ItemInterface(ScrollArea):
    ...
    def __init__(self, sizeHintDyber, parent=None):
        ...
        self.ItemListLink = HyperlinkButton(
            settings.ITEMCOLLECT_LINK, self.tr("Collected Item MOD"), self, FIF.LINK
        )
        ...
        self.instructButton = TransparentPushButton(
            self.tr("Add Items Manually"), self, FIF.QUESTION
        )
    ...
    def __onAddClicked(self):
        ...
        content = self.tr(
            "You are about to import a item MOD from a local file. Please be aware that it is from third-party sources. We are not responsible for any potential harm or issues that may arise from using this MOD. Only proceed if you trust the source."
        )
        if not self.__showMessageBox(title, content):
            return
        ...
        folder = QFileDialog.getExistingDirectory(
            self,
            self.tr("Please select the MOD folder"),
