...
class SaveInterface(ScrollArea):
    ...
    def __init__(self, sizeHintDyber, parent=None):
        ...
        docPath = QStandardPaths.locate(
            QStandardPaths.DocumentsLocation, "", QStandardPaths.LocateDirectory
        )
        ...
        self.ImportSaveCard.ToolButton.setToolTip(
            self.tr("Select pet, then select folder")
        )
        ...
        for iCard in range(6):
            ...
            if os.path.exists(folder):
                ...
                if len(allSaves) > 0:
                    jsonPath = [
                        iSave
                        for iSave in allSaves
                        if iSave.startswith(str(len(allSaves) - 1))
                    ][0]
                    jsonPath = os.path.join(folder, jsonPath)
                    card = QuickSaveCard(
                        iCard, jsonPath=jsonPath, parent=self.QuickSaveGroup
                    )
                else:
    ...
    def __connectSignalToSlot(self):
        ...
        self.ExportSaveCard.clicked.connect(self.__onExportSaveCardClicked)
        self.ImportSaveCard.optionSelcted.connect(self.__onImportSaveCardClicked)
    ...
    def __onImportSaveCardClicked(self, petname):
        ...
        ExportDir = os.path.normpath(docPath + ExportDir)
        folder = QFileDialog.getExistingDirectory(
            self, self.tr("Choose Save Folder to Import"), ExportDir
        )  # self.ImportSaveCard.contentLabel.text())
    ...
    def _loadin_petData(self, folder, petname):
        ...
        if not os.path.exists(os.path.join(folder, "pet_data.json")):
            self.__showSystemNote(
                self.tr("File: pet_data.json not found in selected folder!"),
                type_code=2,
            )
            return
        ...
        save_dict = json.load(
            open(os.path.join(folder, "pet_data.json"), "r", encoding="UTF-8")
        )
        ...
        if not CheckStatus:
            self.__showSystemNote(
                self.tr("File: pet_data.json is not in compatible format!"), type_code=2
            )
            return
        ...
        if TransferStatus:
            self.__showSystemNote(self.tr("Save imported successfully!"), type_code=0)
        else:
            self.__showSystemNote(self.tr("Failed to import save!"), type_code=2)
    ...
    def __onCardLoadinClicked(self, cardIndex):
        ...
        info = open(
            os.path.join(jsonPath, "info.txt"), "r", encoding="UTF-8"
        ).readlines()
    ...
    def __onCardDeleteClicked(self, cardIndex):
        ...
        content = self.tr(
            """All history saves in this slot will be deleted, use carefully"""
        )
        if not self.__showMessageBox(title, content):
            return
    ...
    def __onCardBackClicked(self, cardIndex):
        ...
        content = self.tr(
            """It will delete the current save, and backtrace to the last one in this slot."""
        )
        if not self.__showMessageBox(title, content):
            return
