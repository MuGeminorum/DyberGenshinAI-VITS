...
class DPAccessory(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.acc_dict = {}
        self.heart_list = []
        self.bubble_frame = _load_item_img(
            os.path.join(basedir, "res/role/sys/action/bubble.png")
        )
    ...
    def setup_accessory(self, acc_act, pos_x, pos_y):
        ...
        if acc_act.get("name", "") == "item_drop":
            ...
            self.acc_dict[acc_index] = SubPet(
                acc_index, acc_act["pet_name"], pos_x, pos_y
            )
            ...
            self.acc_dict[acc_index] = SubPet(
                acc_index, acc_act["pet_name"], pos_x, pos_y, isSubpet=True
            )
            ...
            self.acc_dict[acc_index] = DPDialogue(
                acc_index, acc_act["msg_dict"], pos_x, pos_y, closable=True, timeout=-1
            )
            ...
            self.acc_dict[acc_index] = QAccessory(acc_index, acc_act, pos_x, pos_y)

            if acc_act.get("follow_main", False):
                self.send_main_movement.connect(
                    self.acc_dict[acc_index].update_main_pos
                )
...
class QHangLabel(QWidget):
    ...
    def __init__(self, acc_index, acc_act, pos_x, pos_y, parent=None):
...
class QAccessory(QWidget):
    ...
    def __init__(self, acc_index, acc_act, pos_x, pos_y, parent=None):
        ...
        # else:
        self.move(
            pos_x + self.anchor[0] * settings.tunable_scale,
            pos_y + self.anchor[1] * settings.tunable_scale,
        )
        ...
        if self.closable:
            ...
            self.quit_act = Action(FIF.CLOSE, self.tr("Withdraw"), menu)
    ...
    def _move_to_mouse(self, x, y):
        ...
        if self.is_follow_mouse == "x":
            ...
            self.move(
                x - self.anchor[0] * settings.tunable_scale,
                y - self.anchor[1] * settings.tunable_scale,
            )
    ...
    def move_to_main(self):
        ...
        if movement_y != 0:
            ...
            plus_x = int(
                self.speed_follow_main
                * kb
                / ((1 + kb**2) ** 0.5)
                * (int(movement_x > 0) * 2 - 1)
            )
            plus_y = int(
                self.speed_follow_main
                * 1
                / ((1 + kb**2) ** 0.5)
                * (int(movement_y > 0) * 2 - 1)
            )
        else:
        ...
        if max(1, self.speed_follow_main * settings.tunable_scale) >= (
    ...
    def __init__(self, movement=True, click=False, parent=None):
        ...
        if movement and click:
            self._listener = mouse.Listener(
                on_move=self._handle_move, on_click=self._handle_click
            )
        elif movement:
...
class QItemDrop(QWidget):
    ...
    def __init__(self, acc_index, acc_act, pos_x, pos_y, parent=None):
        ...
        self.label = QItemLabel(
            self.acc_act["frame"].scaled(
                self.size_wh,
                self.size_wh,
                aspectMode=Qt.KeepAspectRatio,
                mode=Qt.SmoothTransformation,
...
class SubPet(QWidget):
    ...
    def __init__(self, acc_index, pet_name, pos_x, pos_y, isSubpet=False, parent=None):
        """
        简化的宠物附件
        """
        ...
        if self.follow_main:
            self.move(
                pos_x
                + (settings.current_img.width() // 2 + self.pet_conf.anchor_to_main[0])
                * settings.tunable_scale,
                pos_y
                + (-settings.current_img.height() + self.pet_conf.anchor_to_main[1])
                * settings.tunable_scale,
            )
            self.destination = [
                pos_x
            ...
            self.move(
                int(
                    self.current_screen.topLeft().x()
                    + random.uniform(0.4, 0.7) * self.screen_width
                ),
                self.pos().y(),
            )
        ...
        if event.button() == Qt.LeftButton:
            ...
            if self.onfloor == 1:
                ...
                if self.set_fall == 1:
                    ...
                    self.dragspeedx = (
                        (self.mouseposx1 - self.mouseposx3) / 2 * settings.fixdragspeedx
                    )
                    self.dragspeedy = (
                        (self.mouseposy1 - self.mouseposy3) / 2 * settings.fixdragspeedy
                    )
        ...
        if self.follow_main:
            if self.follow_main_x:
                x_new = (
                    pos_x
                    + (
                        settings.current_img.width() // 2
                        + self.pet_conf.anchor_to_main[0]
                    )
                    * settings.tunable_scale
                )
            else:
    ...
    def reset_size(self):
        self.setFixedSize(
            (self.pet_conf.width + self.margin_value)
            * max(1.0, settings.tunable_scale),
            (self.margin_value + self.pet_conf.height)
            * max(1.0, settings.tunable_scale),
        )
    ...
    def set_img(self):  # , img: QImage) -> None:
        ...
        if self.previous_anchor != self.current_anchor:
            ...
            self.move(
                self.pos().x() - self.previous_anchor[0] + self.current_anchor[0],
        ...
        if self.pet_conf.act_name:  # is not None:
            ...
            if self.curr_pet_name in settings.pets:
                select_acts = [
                    ...
                    if (
                        self.pet_conf.act_type[i][1]
                        <= settings.pet_data.allData_params[self.curr_pet_name][
                            "FV_lvl"
                        ]
                    )
        ...
        if self.pet_conf.acc_name:  # is not None:
            if self.curr_pet_name in settings.pets:
                select_accs = [
                    ...
                    if (
                        self.pet_conf.accessory_act[self.pet_conf.acc_name[i]][
                            "act_type"
                        ][1]
                        <= settings.pet_data.allData_params[self.curr_pet_name][
                            "FV_lvl"
    ...
    def animation(self):
        ...
        if self.interact is None:
            if self.dist_listen:
                distance_to_main = (
                    (self.main_x - self.pos().x() - self.width() / 2) ** 2
                    + (self.main_y - self.pos().y() - self.height() / 2) ** 2
                ) ** 0.5
                for dist_value in self.distance_acts.keys():
                    if (distance_to_main + dist_value < 0) or (
                        distance_to_main > dist_value and dist_value > 0
                    ):
    ...
    def animat(self, act_name):
        ...
        if self.act_id >= len(acts):
            ...
            if (
                self.previous_img != self.current_img
                or self.previous_anchor != self.current_anchor
            ):
                self.set_img()
                self._move(act)
