...
sys_lvl_bar = (
    settings.LVL_BAR
)  # [20, 120, 300, 600, 1200, 1800, 2400, 3200] #Line 134 sys_lvl_bar = [20, 200, 400, 800, 2000, 5000, 8000, 5000, 5000, 5000, 5000]
...
class DP_FvBar(QProgressBar):
    ...
    def init_FV(self, fv_value, fv_lvl):
        ...
        self.setMaximum(self.points_to_lvlup)
        self.setFormat(
            "lv%s: %i/%s" % (int(self.fvlvl), fv_value, self.points_to_lvlup)
        )
    ...
    def _level_up(self, newValue, oldValue, added=0):
        if self.fvlvl == (len(self.lvl_bar) - 1):
            ...
            self.setMaximum(self.points_to_lvlup)
            self.setFormat(
                "lv%s: %s/%s" % (int(self.fvlvl), int(newValue), self.points_to_lvlup)
            )
...
class PetWidget(QWidget):
    ...
    def _setup_compensate(self):
        ...
        self.compensate_timer = QTimer(
            singleShot=True, timeout=self._compensate_rewards
        )
    ...
    def moveEvent(self, event):
        self.move_sig.emit(
            self.pos().x() + self.width() // 2, self.pos().y() + self.height()
        )
        ...
        if event.button() == Qt.LeftButton:
            ...
            if settings.onfloor == 1 and not self.mouse_moving:
                ...
                if settings.set_fall == 1:
                    ...
                    settings.dragspeedx = (
                        (settings.mouseposx1 - settings.mouseposx3)
                        / 2
                        * settings.fixdragspeedx
                    )
                    settings.dragspeedy = (
                        (settings.mouseposy1 - settings.mouseposy3)
                        / 2
                        * settings.fixdragspeedy
                    )
    ...
    def _init_widget(self) -> None:
        ...
        if settings.on_top_hint:
            ...
            if platform == "win32":
                self.setWindowFlags(
                    Qt.FramelessWindowHint | Qt.SubWindow | Qt.NoDropShadowWindowHint
                )
            else:
                # SubWindow not work in MacOS
    ...
    def _set_menu(self, pets=()):
        ...
        self.start_follow_mouse = Action(
            QIcon(os.path.join(basedir, "res/icons/cursor.svg")),
        ...
        if self.pet_conf.act_name is not None:
            ...
            self.select_acts = [
                _build_act(self.pet_conf.act_name[i], self.act_menu, self._show_act)
                for i in range(len(self.pet_conf.act_name))
                if (self.pet_conf.act_type[i][1] <= settings.pet_data.fv_lvl)
                and self.pet_conf.act_name[i] is not None
            ]
            self.act_menu.addActions(self.select_acts)

        if self.pet_conf.acc_name is not None:
            self.select_accs = [
                _build_act(self.pet_conf.acc_name[i], self.act_menu, self._show_acc)
                for i in range(len(self.pet_conf.acc_name))
                if (
                    self.pet_conf.accessory_act[self.pet_conf.acc_name[i]]["act_type"][
                        1
                    ]
        ...
        self.companion_menu.setIcon(
            QIcon(os.path.join(basedir, "res/icons/partner.svg"))
        )

        add_acts = [
            _build_act(name, self.companion_menu, self._add_pet) for name in pets
        ]
        ...
        self.defaultAct_menu.setIcon(
            QIcon(os.path.join(basedir, "res/icons/repeatone.svg"))
        )

        if self.pet_conf.act_name is not None:
            self.default_acts = [
                _build_act(
                    icon=QIcon(os.path.join(basedir, "res/icons/dot.png")),
                    name=self.pet_conf.act_name[i],
                    parent=self.defaultAct_menu,
                    act_func=self._set_defaultAct,
                )
                for i in range(len(self.pet_conf.act_name))
                if (self.pet_conf.act_type[i][1] <= settings.pet_data.fv_lvl)
                and self.pet_conf.act_name[i] is not None
        ...
        self.change_menu.setIcon(
            QIcon(os.path.join(basedir, "res/icons/character.svg"))
        )
        change_acts = [
            _build_act(name, self.change_menu, self._change_pet) for name in pets
        ]
        ...
        # Drop on/off
        if settings.set_fall == 1:
            self.switch_fall = Action(
                QIcon(os.path.join(basedir, "res/icons/on.svg")),
                self.tr("Allow Drop"),
                menu,
            )
        else:
            self.switch_fall = Action(
                QIcon(os.path.join(basedir, "res/icons/off.svg")),
                self.tr("Don't Drop"),
                menu,
            )
        ...
        for i in range(len(self.pet_conf.act_name)):
            ...
            if act_type[1] > settings.pet_data.fv_lvl:
                if act_name in [acti.text() for acti in self.select_acts]:
                    act_index = [acti.text() for acti in self.select_acts].index(
                        act_name
                    )
        ...
        for acc_name in self.pet_conf.acc_name:
            ...
            if acc_type[1] > settings.pet_data.fv_lvl:
                if acc_name in [acci.text() for acci in self.select_accs]:
                    acc_index = [acci.text() for acci in self.select_accs].index(
                        acc_name
                    )
        ...
        for i in range(len(self.pet_conf.act_name)):
            ...
            if act_type[1] > settings.pet_data.fv_lvl:
                if act_name in [acti.text() for acti in self.default_acts]:
                    act_index = [acti.text() for acti in self.default_acts].index(
                        act_name
                    )
        ...
        self.StatMenu.addActions(
            [
                ...
                Action(
                    ...
                    self.tr("Dashboard"),
                    triggered=self._show_dashboard,
                ),
                Action(
                    QIcon(os.path.join(basedir, "res/icons/SystemPanel.png")),
                    self.tr("System"),
                    triggered=self._show_controlPanel,
                ),
                Action(FIF.POWER_BUTTON, self.tr("Exit"), triggered=self.quit),
            ]
        )
    ...
    def _add_pet(self, pet_name: str):
        ...
        self.setup_acc.emit(
            pet_acc, int(self.pos().x() + self.width() / 2), self.pos().y()
        )
    ...
    def _setup_ui(self):
        ...
        settings.current_img = self.pet_conf.default.images[
            0
        ]  # list(pic_dict.values())[0]
    ...
    def reset_size(self, setImg=True):
        ...
        self.setFixedSize(
            int(
                max(
                    self.tomato_time.width() + statbar_h,
                    self.pet_conf.width * settings.tunable_scale,
                )
            ),
    ...
    def set_img(self):  # , img: QImage) -> None:
        ...
        if settings.previous_anchor != settings.current_anchor:
            self.move(
                self.pos().x()
                - settings.previous_anchor[0]
                + settings.current_anchor[0],
                self.pos().y()
                - settings.previous_anchor[1]
                + settings.current_anchor[1],
            )
        ...
        self.label.setPixmap(
            settings.current_img
        )  # QPixmap.fromImage(settings.current_img))
    ...
    def register_accessory(self, accs):
        self.setup_acc.emit(
            accs, self.pos().x() + self.width() // 2, self.pos().y() + self.height()
        )

    def _change_status(
        self, status, change_value, from_mod="Scheduler", send_note=False
    ):
    ...
    def use_item(self, item_name):
        ...
        # 对话物品
            ...
            if item_name in self.pet_conf.msg_dict:
                accs = {
                    "name": "dialogue",
                    "msg_dict": self.pet_conf.msg_dict[item_name],
                }
        ...
        self._change_status(
            "hp",
            self.items_data.item_dict[item_name]["effect_HP"],
            from_mod="inventory",
            send_note=True,
        )
        if item_name in self.pet_conf.item_favorite:
            self._change_status(
                "fv",
                int(
                    self.items_data.item_dict[item_name]["effect_FV"]
                    * self.pet_conf.item_favorite[item_name]
                ),
                from_mod="inventory",
                send_note=True,
            )
            ...
            self._change_status(
                ...
                int(
                    self.items_data.item_dict[item_name]["effect_FV"]
                    * self.pet_conf.item_dislike[item_name]
                ),
                from_mod="inventory",
                send_note=True,
            )
    ...
    def patpat(self):
        ...
        if prob_num_0 < sys_pp_heart:
            ...
            x = (
                self.pos().x()
                + self.width() // 2
                + random.uniform(-0.25, 0.25) * self.label.width()
            )
            y = (
                self.pos().y()
                + self.height()
                - 0.8 * self.label.height()
    ...
    def show_compday(self):
        ...
        if sender.text() == "显示陪伴天数":
            acc = {
                ...
                "message": "这是%s陪伴你的第 %i 天"
                % (settings.petname, settings.pet_data.days),
            }
            sender.setText("关闭陪伴天数")
            x = self.pos().x() + self.width() // 2
            y = (
                self.pos().y() + self.height() - self.label.height() - 20
            )  # *settings.size_factor
    ...
    def show_tomato(self):
        if self.tomato_window.isVisible():
            ...
            self.tomato_window.move(
                max(
                    self.current_screen.topLeft().y(),
                    self.pos().x() - self.tomato_window.width() // 2,
                ),
                max(
                    self.current_screen.topLeft().y(),
                    self.pos().y() - self.tomato_window.height(),
                ),
            )
    ...
    def show_focus(self):
        if self.focus_window.isVisible():
            ...
            self.focus_window.move(
                max(
                    self.current_screen.topLeft().y(),
                    self.pos().x() - self.focus_window.width() // 2,
                ),
                max(
                    self.current_screen.topLeft().y(),
                    self.pos().y() - self.focus_window.height(),
                ),
            )
    ...
    def pause_focus(self, state):
        if state:  # 暂停
            ...
            self.workers["Scheduler"].resume_focus(
                int(self.focus_time.value()), int(self.focus_time.maximum())
            )

    def cancel_focus(self):
        self.workers["Scheduler"].cancel_focus(
            int(self.focus_time.maximum() - self.focus_time.value())
        )
    ...
    def show_remind(self):
        if self.remind_window.isVisible():
            ...
            self.remind_window.move(
                max(
                    self.current_screen.topLeft().y(),
                    self.pos().x() - self.remind_window.width() // 2,
                ),
                max(
                    self.current_screen.topLeft().y(),
    ...
    def show_inventory(self):
        if self.inventory_window.isVisible():
            ...
            self.inventory_window.move(
                max(
                    self.current_screen.topLeft().y(),
                    self.pos().x() - self.inventory_window.width() // 2,
                ),
                max(
                    self.current_screen.topLeft().y(),
    ...
    def runInteraction(self):
        ...
        self.workers["Interaction"].sig_interact_note.connect(
    ...
    def runScheduler(self):
        ...
        self.workers["Scheduler"].sig_settext_sche.connect(
            self.register_notification
        )  # _set_dialogue_dp)
    ...
    def switch_screen(self, screen):
        ...
        self.floor_pos = (
            self.current_screen.topLeft().y() + self.screen_height - self.height()
        )

    def limit_in_screen(self, new_x, new_y):
        # 超出当前屏幕左边界
        if (
            new_x + self.width() // 2 < self.current_screen.topLeft().x()
        ):  # self.border:
            # surpass_x = 'Left'
            new_x = (
                self.current_screen.topLeft().x() - self.width() // 2
            )  # self.screen_width + self.border - self.width()

        # 超出当前屏幕右边界
        elif (
            new_x + self.width() // 2
            > self.current_screen.topLeft().x() + self.screen_width
        ):  # self.current_screen.bottomRight().x(): # + self.border:
            # surpass_x = 'Right'
            new_x = (
        ...
        # 超出当前屏幕上边界
        if (
            new_y + self.height() - self.label.height() // 2
            < self.current_screen.topLeft().y()
        ):  # self.border:
            # surpass_y = 'Top'
            new_y = (
        ...
        if act_name == settings.defaultAct[self.curr_pet_name]:
            ...
            for action in self.defaultAct_menu.menuActions():
                if action.text() == settings.defaultAct[self.curr_pet_name]:
                    ...
                    action.setIcon(
                        QIcon(os.path.join(basedir, "res/icons/dotfill.png"))
                    )  # os.path.join(basedir, 'res/icons/check_icon.png')))
